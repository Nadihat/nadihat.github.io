...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateFrequencyDisplay() {
        const calculatedFrequency = BASE_FREQUENCY * currentMultiplier;
        const displayMultiplier = currentMultiplier.toFixed(2); // Format multiplier
        const displayFreq = calculatedFrequency.toFixed(2);
        frequencyDisplay.textContent = `${displayMultiplier}x - ${displayFreq} Hz`; // Use calculated values
    ...
    function startSound() {
        ...
        if (!oscillator) {
            ...
            oscillator.type = 'sine'; // Force sine waveform
            const targetFrequency = BASE_FREQUENCY * currentMultiplier; // MODIFIED HERE: Direct calculation
   ...
   multiplierSlider.addEventListener('input', () => {
        currentMultiplier = parseFloat(multiplierSlider.value); // MODIFIED HERE
